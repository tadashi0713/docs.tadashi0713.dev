...
このような状況が発生すると、Docker Scout はその差異を検出し、[最新のベースイメージポリシー](/scout-ja/policy#up-to-date-base-images)を通じて通知します。GitHub 統合が有効化されていると、ベースイメージの更新に関する自動的な提案も受けられます。Docker Scout がどのようにしてサプライチェーンのセキュリティ対策を自動的に改善できるかについては、[修正](https://chatgpt.com/policy/remediation.md)を参照してください。
