...
Docker Build Cloud を利用するには、[Docker アカウントを作成](https://docs.docker.com/accounts/create-account/)してください。Docker Build Cloud へのアクセス方法は以下の2つです:

- 無料の個人アカウントをお持ちのユーザーは、7日間の無料トライアルに登録できます。その後、サブスクリプションでアクセスを継続することも可能です。無料トライアルを開始するには、[Docker Build Cloud ダッシュボード](https://app.docker.com/build/)にサインインし、画面の指示に従ってください。

- 有料の Docker サブスクリプションをお持ちのすべてのユーザーは、Docker 製品スイートに含まれる Docker Build Cloud へのアクセスが利用可能です。詳しくは、[Docker のサブスクリプションと機能](https://docs.docker.com/subscription/details/)をご覧ください。
...
サインアップしてビルダーを作成したら、[ローカル環境でビルダーをセットアップ](/build-cloud-ja/setup)して続行してください。

Docker Build Cloud に関連するロールと権限に関する情報は、[ロールと権限](/docker-security-ja/for-admins/roles-and-permissions/#docker-build-cloud)をご覧ください。
