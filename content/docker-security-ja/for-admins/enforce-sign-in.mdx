...
デフォルトでは、Organization のメンバーはサインインせずに Docker Desktop を使用できます。ユーザーが Organization のメンバーとしてサインインしない場合、[Organization のサブスクリプションの特典](https://docs.docker.com/subscription/details/)を受けられず、[Docker のセキュリティ機能](https://docs.docker.com/security/for-admins/hardened-desktop/)を回避できてしまいます。
