...
Docker Scout は[パッケージ URL (PURL)](https://github.com/package-url/purl-spec)を使用して CVE に対するパッケージのマッチングを行うため、脆弱性の識別精度が向上します。PURL を使用することで、誤検出の可能性が大幅に低減され、実際に影響を受けるパッケージのみが対象となります。
