...
<Table>
  ...
  <tbody>
    <Tr>
      <Td>`scout_stream_vulnerabilities`</Td>
      <Td>ストリーム内の脆弱性数</Td>
      <Td>`streamName`, `severity`</Td>
      <Td>Gauge</Td>
    </Tr>
    <Tr>
      <Td>`scout_policy_compliant_images`</Td>
      <Td>ストリーム内でポリシーに準拠しているイメージ数</Td>
      <Td>`id`, `displayName`, `streamName`</Td>
      <Td>Gauge</Td>
    </Tr>
    <Tr>
      <Td>`scout_policy_evaluated_images`</Td>
      <Td>ストリーム内でポリシー評価を行ったイメージ数</Td>
      <Td>`id`, `displayName`, `streamName`</Td>
      <Td>Gauge</Td>
    </Tr>
  </tbody>
